...
m.Reify() |> List.sortBy fst |> Utils.normalize |> Utils.histogram2 20.

(zipf 1.794 100).Reify() |> List.sortByDescending fst |> List.map (fun (p,Value x) -> x, (p * 100e9).ToString("c2", System.Globalization.CultureInfo.CreateSpecificCulture("en-US"))) // |> List.sortByDescending snd
