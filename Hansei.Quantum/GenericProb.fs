...
type GenericProbabilitySpaceSeqBuilder() =
    member inline _.Bind(space, k) = reflect space k
    member inline _.Return v = always v
    member inline _.ReturnFrom vs = vs
    member inline _.Yield x = always x
    member _.Zero() = []
    member inline _.Delay(f: unit -> GenericProbabilitySpace<'a,'W>) =
        [ ContinuedSubTree(memo f), 'W.One ]
    member inline this.Combine(a, b) = this.Bind(a, fun _ -> b)
    member inline this.For(sequence: seq<'a>, body: 'a -> GenericProbabilitySpace<'b,'W>) =
        let e = sequence.GetEnumerator()
        let rec loop () =
            if e.MoveNext() then this.Bind(body e.Current, fun _ -> loop()) else fail()
        loop()
...
let distSeq = GenericProbabilitySpaceSeqBuilder()
